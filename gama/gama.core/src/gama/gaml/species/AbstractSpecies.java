...
public abstract class AbstractSpecies extends Symbol implements ISpecies {
	...
	public IList<ISpecies> getSubSpecies(final IScope scope) {
		...
		var auto = scope.getModel();
		
		DEBUG.OUT("auto.getAllSpecies() " + auto.getAllSpecies());
		DEBUG.OUT("GamlModelSpecies model " + model);
		DEBUG.OUT("this s " + this);
		for (final ISpecies s : model.getAllSpecies().values()) {
			DEBUG.OUT("ISpecies s " + s);
			DEBUG.OUT("s.getParentSpecies() " + s.getParentSpecies());
			if (s.getParentSpecies() == this) { 
				DEBUG.OUT("getParentSpecies true ");
				subspecies.add(s); 
			}
		}
		DEBUG.OUT("subspecies end " + subspecies);
		return subspecies;
	}
