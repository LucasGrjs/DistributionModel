...
public class ImageDisplaySurface implements IDisplaySurface {
	...
	public ImageDisplaySurface(final Object... args) {
		...
		DEBUG.OUT("Image Display Surface created for simulation " + output.getScope().getSimulation());
		data = output.getData();
	...
	public GamaImage getImage(final int w, final int h) {
		DEBUG.OUT("Asking to snapshot step " + scope.getClock().getCycle() + "  from " + Thread.currentThread()
