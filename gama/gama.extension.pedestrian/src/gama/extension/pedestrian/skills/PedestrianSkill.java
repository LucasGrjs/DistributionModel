...
public class PedestrianSkill extends MovingSkill {
	...
	public IPath primComputeVirtualPath(final IScope scope) throws GamaRuntimeException {
		...
		DEBUG.OUT("primComputeVirtualPath 1");
		...
		DEBUG.OUT("primComputeVirtualPath 3");

		DEBUG.OUT("segments.size() " + segments.size());
		...
		DEBUG.OUT("primComputeVirtualPath 4 " + targets);
		DEBUG.OUT("thePath " + thePath);
		DEBUG.OUT("targets.size() " + targets.size());
		if(targets.size() <= 0)
		{
			DEBUG.OUT("return ing thePath " + thePath);
			return thePath;
		}
		IShape targ = targets.get(0);
		DEBUG.OUT("primComputeVirtualPath 4.4 ");
		IAgent road = (IAgent) roadTarget.get(targ);
		DEBUG.OUT("primComputeVirtualPath 4.5 ");
		if (road != null) { 
			DEBUG.OUT("road != null");
		...
		DEBUG.OUT("primComputeVirtualPath 6");

		agent.setAttribute(CURRENT_PATH, thePath);
		DEBUG.OUT("primComputeVirtualPath 7");
		return thePath;
	}
